...
/**
 * New primary similarity function.
 * Returns score 0-100.
 */
export function calculateHardwareAwareSimilarity(report: Report, userRig: UserPC | null): number {
  if (!userRig) return 0;

  let score = 45; // slightly lower base than old (old started at 50)

  // === GPU perfIndex distance (dominant signal — ~75% of value) ===
  ...
  if (reportGpuPerf != null && userGpuPerf != null) {
    ...
    // Within 8 points = extremely similar class of card
    if (delta <= 5) score += 32;
    ...
    // Fallback to old series heuristic when no perf data
  ...
  // Resolution context (future: could weight more)
