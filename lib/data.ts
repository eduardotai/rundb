...
export type GamesBrowseSort = 'reports' | 'name' | 'year'

export function applyGamesBrowseTransform(
  games: Game[],
  gameStatsMap: Record<string, GameStats> = {},
  options: { tier?: PerformanceTier | ''; sort?: GamesBrowseSort } = {}
): Game[] {
  let result = [...games]
  ...
  if (options.tier) {
    result = result.filter((g) => {
      ...
      return dominant === options.tier
    })
  }

  if (options.sort === 'reports') {
    result.sort((a, b) => {
      const byReports = (gameStatsMap[b.id]?.totalReports ?? 0) - (gameStatsMap[a.id]?.totalReports ?? 0)
      return byReports || a.name.localeCompare(b.name)
    })
  } else if (options.sort === 'name') {
    result.sort((a, b) => a.name.localeCompare(b.name))
  } else if (options.sort === 'year') {
    result.sort((a, b) => (b.releaseYear ?? 0) - (a.releaseYear ?? 0) || a.name.localeCompare(b.name))
  }
...
async function getStarterGamesPage(params: Required<Pick<GetGamesPageParams, 'page' | 'pageSize' | 'sort'>> & Pick<GetGamesPageParams, 'search' | 'genre'>): Promise<GamesPageResult> {
  ...
  const reportsByGame: Record<string, Report[]> = {}
  for (const report of mock.getAllReports()) {
    ;(reportsByGame[report.gameId] ??= []).push(report)
  }
  const statsByGame = Object.fromEntries(
    Object.entries(reportsByGame).map(([gameId, reports]) => [gameId, computeGameStatsFromReports(reports)])
  )
  games = applyGamesBrowseTransform(games, statsByGame, { sort: params.sort })
...
export async function getGamesPage(params: GetGamesPageParams = {}): Promise<GamesPageResult> {
  ...
  const pageSize = Math.min(10000, Math.max(1, params.pageSize ?? 48))
  const search = params.search?.trim() ?? ''
  const genre = params.genre?.trim() ?? ''
  ...
  else if (sort === 'reports') query = query.order('report_count', { ascending: false }).order('name', { ascending: true })
