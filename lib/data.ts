...
async function getStarterGamesPage(params: Required<Pick<GetGamesPageParams, 'page' | 'pageSize' | 'sort'>> & Pick<GetGamesPageParams, 'search' | 'genre'>): Promise<GamesPageResult> {
  ...
  if (params.sort === 'reports') {
    ...
    for (const report of mock.getAllReports()) {
      reportCounts.set(report.gameId, (reportCounts.get(report.gameId) ?? 0) + 1)
    }
    games.sort(
      (a, b) =>
        (reportCounts.get(b.id) ?? 0) - (reportCounts.get(a.id) ?? 0) ||
        a.name.localeCompare(b.name)
    )
  } else if (params.sort === 'name') games.sort((a, b) => a.name.localeCompare(b.name))
