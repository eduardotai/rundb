...
- `backfill-steam-requirements.ts`: fill `games.official_min_reqs` / `official_rec_reqs` from Steam `appdetails` `pc_requirements`. Store API rate-limits hard — default 1.5s delay, 429 backoff, stop-after consecutive limits, safe resume via only-missing (`--delay-ms`, `--limit`, `--dry-run`, `--force`).
