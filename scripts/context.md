...
- `backfill-steam-requirements.ts`: optional warm-cache fill of `games.official_min_reqs` / `official_rec_reqs` from Steam `appdetails`. Prefer lazy ensure on game detail for the long tail. Store API rate-limits hard — default 1.5s delay, 429 backoff, stop-after consecutive limits, safe resume via only-missing (`--delay-ms`, `--limit`, `--dry-run`, `--force`).
- `backfill-steam-app-ids.ts` (`npm run backfill:steam-appids`): fill null `games.steam_app_id` via Steam-first resolver. Lazy ensure also links missing AppIDs on game open.
