...
function FooterLink({ href, label, external, children }: FooterLinkProps) {
  const className = cn(iconLinkClass);
...
function FooterIconStandby({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <span
      role="img"
      aria-label={label}
      title="Coming soon"
      className={cn(iconLinkClass, "cursor-not-allowed opacity-40")}
    >
      {children}
    </span>
  );
    ...
    <footer className="border-t border-border py-3">
      <div className="mx-auto flex max-w-7xl flex-col items-center gap-1 px-4 text-center">
        ...
        <p className="text-[10px] leading-tight text-muted-foreground">
          Real PC performance data, by the community.
        </p>
        ...
        <nav aria-label="Footer links" className="flex items-center gap-0.5">
          ...
          <FooterLink href="https://x.com/taisalless" label="X" external>
            <X className="h-3 w-3" aria-hidden="true" />
          </FooterLink>
          <FooterIconStandby label="Discord (coming soon)">
